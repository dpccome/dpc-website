...
(function ($, undefined) {
	...
	var keys = (function () {
		...
		$.each(keys.split(''), function (index, value) {
			...
			if (shift)
				defs[shift] = {
					keyCode: keyCode,
					charCode: shift.charCodeAt(0),
					shift: value,
					requiresShift: index >= 26
				};
		});
	...
	var KeyMasher = function (elm) {
		...
		var queueModifierEvent = function (direction, modifier, isForced) {
			...
			var event = $.extend($.Event(), modifierState, {
				type: 'key' + direction,
				keyCode: keys[modifier].keyCode,
				charCode: 0
			});
			elm.trigger(event);
		};
		...
		var queueStroke = function (key) {
			...
			var ignore = !key.charCode || modifierState.alt || modifierState.ctrl || modifierState.meta,
				down = $.extend($.Event('keydown'), modifierState, {
					keyCode: key.keyCode,
					charCode: 0,
					which: key.keyCode
				}),
				press = $.extend($.Event('keypress'), modifierState, {
